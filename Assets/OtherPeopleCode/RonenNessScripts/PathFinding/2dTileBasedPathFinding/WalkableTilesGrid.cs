...
namespace NesScripts.Controls.PathFind
{
    ...
    public class WalkableTilesGrid 
    {
        ...
        ///     < 1.0f = cheap tile.
        /// </param>
        public WalkableTilesGrid(float[,] tiles_costs)
        {
            // create nodes
        ...
        public WalkableTilesGrid(bool[,] walkable_tiles)
        {
            // create nodes
        ...
        public System.Collections.IEnumerable GetNeighbours(Node node, Pathfinding.DistanceType distanceType)
        {
            ...
            switch (distanceType)
            {
                case Pathfinding.DistanceType.Manhattan:
                    ...
                    for (x = -1; x <= 1; ++x)
                    {
                        var neighbor = AddNodeNeighbour(x, y, node, distanceType);
                        if (neighbor != null)
                ...
                case Pathfinding.DistanceType.NoCornerCutting:
                    for (x = -1; x <= 1; x++)
                    {
                        for (y = -1; y <= 1; y++)
                        {
        ...
        Node AddNodeNeighbour(int x, int y, Node node, Pathfinding.DistanceType distanceType)
        {
            // In here, I, Talist, am checking the distanceType to see wether we need to be concerned with corner cutting.
            if (x == 0 && y == 0)
            {
                return null;
            }
            ...
            {
                if (distanceType == Pathfinding.DistanceType.NoCornerCutting) // Check if both cardinal directions are walkable
                {
